...
//     * @param withPremit if true, call premit function, otherwise use the old way to transfer token through 2 approve then transfer from
...
//         address nFTContract,
//         uint256 tokenId,
//         uint256 listingPrice, bool withPremit,
//          uint256 deadline,
//         uint8 v,
//         bytes32 r,
//         bytes32 s
...
//         bool sellForEnabled,
//         uint256 sellFor,
//         uint256 duration, bool withPremit,
//          uint256 deadline,
//         uint8 v,
//         bytes32 r,
//         bytes32 s
...
//         bytes32 r,
//         bytes32 s
//     ) internal  returns (bytes32 listId)  {
//         listId = keccak256(abi.encodePacked(nFTContract, tokenId, _msgSender(), releaseTime));
...
//      */
//     function fullfillBid(bytes32 listingId, bool withPremit,
//          uint256 deadline,
//         uint8 v,
//         bytes32 r,
//         bytes32 s)
...
//         require(  _permit(_msgSender(),  bidPrice, deadline, v, r, s),'StartFi: Invalid signature');
...
//            _fullfillBid(winnerBidder,    bidPrice,   listingId) ;
...
//      */
//     function buyNow(bytes32 listingId, uint256 price, bool withPremit,
//          uint256 deadline,
//         uint8 v,
//         bytes32 r,
//         bytes32 s) external {
...
//                 (_tokenListings[listingId].status == ListingStatus.onAuction &&
//                     _tokenListings[listingId].sellForEnabled == true &&
//                     _tokenListings[listingId].releaseTime > block.timestamp),
...
//         );
//         // transfer price
//         require(_buyNow(_msgSender(),  price,   listingId,   _tokenListings[listingId].sellForEnabled),'ERROR in Purchase ');
 
       
//         // if bid time is less than 15 min, increase by 15 min
//         // retuen bid id
//     }
//     function _excutePurchase(address buyer, address _NFTContract, uint256 tokenId,uint256 price,  address seller) internal returns (address issuer, uint256 royaltyAmount, uint256 fees, uint256 netPrice) {
//         // transfer price

//         (  issuer,   royaltyAmount,   fees,   netPrice) = _getListingFinancialInfo(
//             _NFTContract,
//             tokenId,
//             price
//         );
...
//     }
//     function _buyNow(address buyer, uint256 price,  bytes32 listingId,  bool sellForEnabled) internal returns (bool) {
...
//         (address issuer, uint256 royaltyAmount, uint256 fees, uint256 netPrice) =_excutePurchase( buyer,   _NFTContract,   tokenId,  price,    seller) ;
//         //  uint256 ListingQualAmount = _getListingQualAmount(_tokenListings[listingId].listingPrice);

//         require(_updateUserReserves(seller, _getListingQualAmount(_tokenListings[listingId].listingPrice), false) >= 0, 'negative reserve is not allowed');

//         // finish listing
//         _finalizeListing(listingId, buyer, ListingStatus.Sold);
//         // TODO: add reputation points to both seller and buyer
//         _addreputationPoints(seller, buyer, price);
//         emit BuyNow(
//             listingId,
//             _NFTContract,
//             buyer,
...
//         return true;
// }
//     function _fullfillBid(address buyer,uint256 price,  bytes32 listingId) internal returns (bool) {
//         address seller = _tokenListings[listingId].seller;
//           address _NFTContract = _tokenListings[listingId].nFTContract;
//        uint256 tokenId = _tokenListings[listingId].tokenId;
//         // transfer price
...
//         (address issuer, uint256 royaltyAmount, uint256 fees, uint256 netPrice) =_excutePurchase( buyer,   _NFTContract,   tokenId,  price,    seller) ;
//         require(
//             _updateUserReserves(buyer, _tokenListings[listingId].qualifyAmount, false) >= 0,
