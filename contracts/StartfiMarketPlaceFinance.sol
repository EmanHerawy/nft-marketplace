...
contract StartfiMarketPlaceFinance is MarketPlaceBase {
    ...
    constructor(string memory _name, address _paymentContract)
    ...
    function _calcSum(uint256 a, uint256 b)
        internal
        pure
    ...
    *@return amount the value that the platform will get
     */
    function _getListingQualAmount(uint256 listingPrice)
        internal
        view
        returns (uint256 amount)
    {
        amount = listingPrice.mul(listqualifyPercentage).div(
    ...
    *@return remaining the value remaing after subtracting the fine
    ...
    {
        fineAmount = listingPrice.mul(delistFeesPercentage).div(
    ...
    * @return remaining the value that the auction woner will get
     */
    function _calcBidDisputeFees(uint256 qualifyAmount)
        internal
        view
        returns (uint256 fineAmount, uint256 remaining)
    {
        fineAmount = qualifyAmount.mul(bidPenaltyPercentage).div(
        ...
        )
    {
        ...
        if (_supportRoyalty(_NFTContract)) {
            (issuer, royaltyAmount) = _getRoyaltyInfo(
                _NFTContract,
                tokenId,
                bidPrice
            );
    ...
    * @return allowed number of tokens that this contract can transfer from the owner account
    ...
    function _getStakeAllowance(
        ...
        return
            IStartFiStakes(stakeContract).getReserves(staker).sub(
                userActualReserved
            );
    }
     ...
     */
    function _setUserReserves(address user, uint256 newReservedValue)
        internal
        returns (bool)
    {
    ...
    function _updateUserReserves(
        ...
        _userReserves = isAddition
            ? userReserves[user].add(newReserves)
            : userReserves[user].sub(newReserves);
     ...
     */
    function _changeFees(uint256 newFees, uint256 newBase)
        internal
     ...
     * @return percentage the value of the state variable `_feeFraction`
     */
    function _changeBidPenaltyPercentage(uint256 newFees, uint256 newBase)
        internal
        returns (uint256 percentage)
    {
        require(newFees <= newBase, "Fee fraction exceeded base.");
    ...
    function _changeDelistFeesPerentage(uint256 newFees, uint256 newBase)
        internal
        returns (uint256 percentage)
    {
        require(newFees <= newBase, "Fee fraction exceeded base.");
     ...
     * @return percentage the value of the state variable `_feeFraction`
     */
    function _changeListqualifyAmount(uint256 newFees, uint256 newBase)
        internal
        returns (uint256 percentage)
    {
        require(newFees <= newBase, "Fee fraction exceeded base.");
