...
contract StartFiNFTPayment is Ownable {
    ...
    * @return allowed number of tokens that this contract can transfer from the owner account.
    ...
    function MintNFTWithRoyalty(
        ...
        return
            IERC721RoyaltyMinter(_NFTToken).mintWithRoyalty(
                to,
                _tokenURI,
                share,
                base
     ...
     * @return token id .
     */
    function MintNFTWithoutRoyalty(address to, string memory _tokenURI)
        external
        returns (uint256)
    {
        require(_getAllowance(_msgSender()) >= _fees, "Not enough fees paid");
     ...
     */
    function changePaymentContract(address _paymentContractAddress)
        external
        onlyOwner
    {
