 ...
 *
 */
contract StartFiMarketPlace is
    Ownable,
    Pausable,
    MarketPlaceListing,
    MarketPlaceBid,
    StartfiMarketPlaceFinance
{
    ...
    modifier canFullfilBid(bytes32 listingId) {
        require(
            _tokenListings[listingId].releaseTime < block.timestamp &&
                _tokenListings[listingId].status != ListingStatus.onAuction,
            "Auction is ended"
    ...
    function listOnMarketplace(
        ...
        listId = keccak256(
            abi.encodePacked(nFTContract, tokenId, _msgSender(), releaseTime)
        );
    ...
    function bid(
        ...
        bidId = keccak256(
            abi.encodePacked(listingId, tokenAddress, _msgSender(), tokenId)
        );
        // bid should be more than the mini and more than the last bid
        ...
        if (lastbidder == address(0)) {
            ...
            require(
                bidPrice > listingBids[listingId][lastbidder].bidPrice,
        ...
        if (prevAmount == 0) {
            ...
            require(
                _updateUserReserves(_msgSender(), qualifyAmount, true) > 0,
                "Reserve Zero is not allowed"
            );
        }
     ...
     */
    function fullfilBid(bytes32 listingId)
        external
        canFullfilBid(listingId)
        returns (address _NFTContract, uint256 tokenId)
    {
        ...
        (
            ...
            uint256 fees,
            uint256 netPrice
        ) = _getListingFinancialInfo(_NFTContract, tokenId, bidPrice);

        require(
            _safeTokenTransferFrom(owner(), buyer, fees),
            "Couldn't transfer token as fees"
        );
        if (issuer != address(0)) {
            require(
                _safeTokenTransferFrom(issuer, buyer, royaltyAmount),
        ...
        require(
            _safeTokenTransferFrom(winnerBidder, buyer, netPrice),
            "Couldn't transfer token to buyer"
        );
        // trnasfer token
        require(
            _safeNFTTransfer(
        ...
        returns (address _NFTContract, uint256 tokenId)
    {
        ...
        require(
            (timeToDelistAuction <= block.timestamp &&
                status == ListingStatus.onAuction) ||
        ...
        if (releaseTime < block.timestamp) {
            ...
            require(
                _deduct(owner, getAdminWallet(), fineAmount),
        ...
        // trnasfer token
        require(
            _safeNFTTransfer(_NFTContract, tokenId, address(this), owner),
            "NFT token couldn't be transfered"
        );
        // update user reserves
        // reserve nigative couldn't be at any case
        require(
            _updateUserReserves(_msgSender(), remaining, false) >= 0,
            "negative reserve is not allowed"
        );
        // finish listing
        ...
        returns (address _NFTContract, uint256 tokenId)
    {
        ...
        (
            ...
            uint256 fees,
            uint256 netPrice
        ) = _getListingFinancialInfo(_NFTContract, tokenId, price);
        ...
        // token value could be zero ater taking the roylty share ??? need to ask?
        require(
            _safeTokenTransferFrom(_msgSender(), buyer, netPrice),
            "Couldn't transfer token to buyer"
        );
        // trnasfer token
        require(
        ...
        uint256 ListingQualAmount = _getListingQualAmount(
            _tokenListings[listingId].listingPrice
        );

        require(
            _updateUserReserves(buyer, ListingQualAmount, false) >= 0,
            "negative reserve is not allowed"
        );
        ...
        returns (address _NFTContract, uint256 tokenId)
    {
        ...
        tokenId = _tokenListings[listingId].tokenId;
        uint256 qualifyAmount = _tokenListings[listingId].qualifyAmount;
        uint256 timeToDispute = _calcSum(
            _tokenListings[listingId].releaseTime,
            3 days
        );
        require(
            winnerBidder != address(0) && timeToDispute >= block.timestamp,
            "No bids or still running auction"
        );
        require(buyer == _msgSender(), "Caller is not the owner");
        require(
            !listingBids[listingId][winnerBidder].isPurchased,
            "Already purchased"
        );
        // call staking contract to deduct
        (uint256 fineAmount, uint256 remaining) = _calcBidDisputeFees(
            qualifyAmount
        );
        require(
            _deduct(winnerBidder, getAdminWallet(), fineAmount),
            "couldn't deduct the fine for the admin wallet"
        );
        require(
            _deduct(winnerBidder, buyer, remaining),
            "couldn't deduct the fine for the admin wallet"
        );
        // trnasfer token
        require(
        ...
        require(
            _updateUserReserves(winnerBidder, qualifyAmount, false) >= 0,
            "negative reserve is not allowed"
        );
    ...
    * @return curentReserves user reserves after freeing the unused reservd
    ...
    function freeReserves() external returns (uint256 curentReserves) {
        ...
        for (uint256 index = 0; index < listings.length; index++) {
            if (
                _tokenListings[listings[index]].status ==
                ListingStatus.onAuction
            ) {
                newListings.push(listings[index]);
                curentReserves = _calcSum(
                    curentReserves,
                ...
                newListings.push(listings[index]);
                uint256 listQualifyAmount = _getListingQualAmount(
        ...
        require(
            _setUserReserves(_msgSender(), curentReserves),
            "set reserve faild"
        );
